 ...
 * <p>这是 Bean 容器的基本客户端视图;诸如 {@link ListableBeanFactory}
 * 和 {@link org.springframework.beans.factory.config.ConfigurableBeanFactory} 等接口可用于特定目的。
 *
 ...
 * <p>此接口由包含多个 bean 定义的对象实现，每个 bean 定义都由 String 名称唯一标识。
 * 根据 Bean 的定义，工厂将返回包含对象的独立实例（原型设计模式）或单个共享实例（单例设计模式的更好替代方案，
 * 在该模式中，实例是工厂范围内的单例）。返回哪种类型的实例取决于 Bean 工厂配置：API 是相同的。
 * 从 Spring 2.0 开始，根据具体的应用程序上下文（例如，Web 环境中的 “request” 和 “session” 范围），可以使用更多范围。
 *
 ...
 * <p>这种方法的要点是 BeanFactory 是应用程序组件的中央注册表，
 * 并集中配置应用程序组件（例如，单个对象不再需要读取属性文件）。
 ...
 * <p>请注意，通常最好依靠依赖注入（“推送”配置）通过 setter 或构造函数来配置应用程序对象，
 * 而不是使用任何形式的“拉取”配置，例如 BeanFactory 查找。
 * Spring 的依赖注入功能是使用此 BeanFactory 接口及其子接口实现的。
 *
 ...
 * <p>与 {@link ListableBeanFactory} 中的方法相反，此接口中的所有操作还将检查父工厂是否为 {@link HierarchicalBeanFactory}。
 * 如果在此工厂实例中找不到 Bean，则将询问直接父工厂。此工厂实例中的 Bean 应该覆盖任何父工厂中的同名 Bean。
 *
 * <p>Bean factory implementations should support the standard bean lifecycle interfaces
 * as far as possible. The full set of initialization methods and their standard order is:
 * <p>Bean 工厂实现应尽可能支持标准的 Bean 生命周期接口。完整的初始化方法集及其标准顺序为：
 * <ol>
...
public interface BeanFactory {
	 ...
	 *             Bean 实例时要使用的 args 参数 使用显式参数（仅在创建新实例时应用，而不是检索现有实例）
	 * @return an instance of the bean
	 * @throws NoSuchBeanDefinitionException if there is no such bean definition
