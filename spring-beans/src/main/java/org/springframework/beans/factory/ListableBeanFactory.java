 ...
 * this interface.
 * 扩展 {@link BeanFactory} 接口，由 Bean 工厂实现，可以枚举其所有 Bean 实例，
 * 而不是根据客户端的要求逐个尝试按名称查找 Bean。
 ...
 * <p>如果这是一个 {@link HierarchicalBeanFactory}，则返回值<i>将不<i>考虑任何 BeanFactory 层次结构，
 * 而仅与当前工厂中定义的 Bean 相关。使用 {@link BeanFactoryUtils} 帮助程序类来考虑祖先工厂中的 bean。
 *
 ...
 * <p>此接口中的方法将仅遵循此工厂的 bean 定义。他们将忽略通过其他方式注册的任何单例 bean，
 ...
 * 当然，BeanFactory 的 {@code getBean} 也确实允许透明地访问这些特殊的 bean。
 * 但是，在典型场景中，所有 Bean 都将由外部 Bean 定义定义，因此大多数应用程序无需担心这种区分。
 *
 * <p><b>NOTE:</b> With the exception of {@code getBeanDefinitionCount}
 * and {@code containsBeanDefinition}, the methods in this interface
 * are not designed for frequent invocation. Implementations may be slow.
 * <p><b>注意：<b>除了 {@code getBeanDefinitionCount} 和 {@code containsBeanDefinition} 之外，
 * 此接口中的方法不是为频繁调用而设计的。实施速度可能会很慢。
 *
 * @author Rod Johnson
