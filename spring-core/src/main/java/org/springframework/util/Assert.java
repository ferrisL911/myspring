 ...
 * <p>有助于在运行时及早、清晰地识别程序员错误。
 *
 * <p>For example, if the contract of a public method states it does not
 * allow {@code null} arguments, {@code Assert} can be used to validate that
 * contract. Doing this clearly indicates a contract violation when it
 * occurs and protects the class's invariants.
 *<p>例如，如果公共方法的协定声明不允许 {@code null} 参数，则可以使用 {@code Assert} 来验证该协定。
 * 这样做清楚地表明了合同违约的情况，当它发生时，并保护了类的不变量。
 *
 *
 ...
 * 与配置初始化代码相比，在此类方法中回退到默认值通常没有意义。
 *
 * <p>This class is similar to JUnit's assertion library. If an argument value is
 * deemed invalid, an {@link IllegalArgumentException} is thrown (typically).
 * For example:
 * <p>此类类似于 JUnit 的断言库。如果参数值被视为无效，则会抛出 {@link IllegalArgumentException}（通常）。例如：
 *
 * <pre class="code">
 ...
 *
 * <p>主要用于框架内的内部使用;对于更全面的断言工具套件，请考虑
 *
 * @author Keith Donald
